import { createNewTopic } from "../lib/fs";
import * as model from "../lib/model";
import * as cmp from "./components";

export class ExplorerView implements model.ChapterObserver, model.TopicObserver {
    ...
    public onChapterCreated(chapter: model.Chapter): void {
        ...
        if (chapterListElement !== null) {
            ...
            chapterListElement.id = this.getTopicNodeIdentifier(chapter.getTopic()) + "-chplist";
            topicElem.appendChild(chapterListElement);
        }
        chapterListElement.appendChild(this.createChapterElement(chapter, ownerDocument));
    }
    ...
    private createTopicElement(topic: model.Topic, ownerDocument: Document) {
        ...
        const topicElem = cmp.createListItem(
            ...
            this.getTopicNodeIdentifier(topic), (ev) => {
    ...
    private createChapterElement(chapter: model.Chapter, ownerDocument: Document): Element {
        ...
        linkElem.appendChild(ownerDocument.createTextNode(chapter.getDisplayName()));
        linkElem.addEventListener("click", () => {
            linkElem.dispatchEvent(new CustomEvent<model.Chapter>(
                'chapterChanged', {
                detail: chapter,
                bubbles: true,
                cancelable: false,
